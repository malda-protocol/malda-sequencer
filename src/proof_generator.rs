...
impl ProofGenerator {
    ...
    pub async fn new(config: ProofGeneratorConfig, db: Database) -> Result<()> {
        info!("Starting proof generator with batch size {}", config.batch_size);
        ...
        info!("Proof generator started, polling for events every {:?}", proof_delay);
        ...
        loop {
            ...
            if !events.is_empty() {
                info!("Processing batch of {} events for proof generation", events.len());
    ...
    async fn get_ready_events(
        ...
        db.get_ready_to_request_proof_events(
    ...
    async fn process_batch(
        ...
        let (users, markets, dst_chain_ids, src_chain_ids) = Self::group_events_by_chain(events.clone());
        
        debug!("Starting batch proof generation for {} source chains", src_chain_ids.len());

        // Generate proof with retry logic and fallback methods
        let proof_info = Self::generate_proof(
            users,
            markets,
            dst_chain_ids,
            src_chain_ids,
    ...
    ) -> (Vec<Vec<Address>>, Vec<Vec<Address>>, Vec<Vec<u64>>, Vec<u64>) {
    ...
    async fn try_boundless_proof(
        ...
        {
            Ok((journal, seal)) => {
                info!("Successfully generated boundless proof for {} transactions", tx_num);
            ...
            Err(e) => {
                warn!("Boundless proof generation failed: {}. Falling back to SDK method", e);
...
///
    ...
    fn create_proof_info_from_sdk(proof_info: malda_rs::viewcalls::MaldaProveInfo) -> Result<ProofInfo> {
