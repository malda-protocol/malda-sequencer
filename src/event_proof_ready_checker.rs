...
impl EventProofReadyChecker {
    ...
    async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            if let Some(optimism_state) = chain_providers.get_mut(&optimism_chain_id) {
                ...
                let (optimism_provider, is_fallback) =
                    match Self::get_active_provider(optimism_state, &self.db).await {
            ...
            match self
                .db
                .update_events_to_ready_status(&current_block_map)
                .await
            {
    ...
    async fn get_active_provider(
        ...
        if !state.used_fallback && state.primary_provider.is_some() {
            ...
            match provider.get_block_by_number(BlockNumberOrTag::Latest).await {
                Ok(Some(block)) => {
                    ...
                    debug!(
                        "Reusing existing primary provider for chain {}",
                        state.config.chain_id
                    );
                ...
                Err(e) => {
                    let reason = format!(
                        "Existing primary provider failed to get latest block: {:?}",
                        e
                    );
                    warn!(
                        "{} for chain {}, creating new one",
        ...
        debug!(
            "Connecting to primary provider for chain {} at {}",
            state.config.chain_id, rpc_url
        );
    ...
    async fn create_fallback_provider(
        ...
        info!(
            "Connecting to fallback provider for chain {} at {}",
            config.chain_id, fallback_url
        );
        let fallback_provider = create_provider(fallback_url)
            .await
...
async fn create_provider(rpc_url: Url) -> Result<ProviderType> {
