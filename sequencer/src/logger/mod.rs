...
use alloy::primitives::{TxHash, Address, U256};
use tracing::error;
...
impl PipelineLogger {
    pub async fn new(file_path: PathBuf) -> Result<Self> {
        ...
        Ok(Self { event_sender, log_path: file_path })
    }
    ...
    async fn log_writer(mut receiver: mpsc::Receiver<LogEvent>, file_path: PathBuf) -> Result<()> {
        ...
        while let Some(event) = receiver.recv().await {
            match &event.step {
                PipelineStep::EventReceived { chain_id, block_number, market, event_type } => {
                ...
                PipelineStep::EventProcessed { chain_id: _, dst_chain_id, market: _, event_type: _, amount } => {
                ...
                PipelineStep::ProofGenerated { duration_ms, journal, seal } => {
                    if let Some((_, ref mut entry)) = pending_logs.get_mut(&event.tx_hash) {
                ...
                PipelineStep::TransactionSubmitted { tx_hash: new_tx_hash, method, gas_used, gas_price } => {
                ...
                PipelineStep::TransactionVerified { tx_hash: new_tx_hash, method, block_number, status } => {
                    if let Some((_, ref entry)) = pending_logs.get(&event.tx_hash) {
